(function(){$.prototype.tipPop=function(c){var d,a,b,e=this;if(c==null){c={};}d=this;b={destroy:function(){this.destroyFocus();return this.destroyHover();
},destroyHover:function(){d.off("mouseenter");d.off("mouseleave");d.data("hover",false);d.find("div#tippop").hide();return a.m.settings.type="focus";},destroyFocus:function(){d.find("div#tippop_focus").each(function(){return $.removeData($(this).data);
...
if(f==null){f={};}g={hoverPosition:"best",focusPosition:"best",type:"all",delay:2000,offset:8,hoverPriority:["top","bottom","left","right"],focusPriority:["left","right","bottom","top"]};
...
break;case"top":g=a.o.getNormalize(j.$elem,j.type);g.normTipPop.addClass("top").css({top:f-g.normH-a.m.settings.offset-4,left:j.$elem.offset().left+(j.$elem.outerWidth()/2)-(g.normW/2)-5}).stop(true,false).fadeIn(500,l);
break;case"bottom":g=a.o.getNormalize(j.$elem,j.type);g.normTipPop.addClass("bottom").css({top:f+j.$elem.outerHeight()+a.m.settings.offset,left:j.$elem.offset().left+(j.$elem.outerWidth()/2)-(g.normW/2)-5}).stop(true,false).fadeIn(500,l);
...
}if(h.data().allowPopOnHover&&(f.toLowerCase()!=="true")){a.m.state.tryPositionCount=0;if(h.attr("title")!=null){h.data("titleText",h.attr("title"));a.m.settings.$tipPop.text(h.data().titleText).css({height:a.m.settings.$tipPop.height()});
